...
@Component({
  ...
  template: `
    ...
    >
      ...
      <div
        *ngIf="isDragging"
        class="fixed inset-0 bg-blue-500/20 border-4 border-dashed border-blue-500 z-50 flex items-center justify-center pointer-events-none animate-fade-in"
      >
        <div class="bg-white rounded-lg p-6 shadow-lg text-center">
          <div
            class="w-16 h-16 mx-auto mb-3 bg-blue-500 rounded-full flex items-center justify-center"
          >
          ...
          <button
            ...
            (dragover)="
              breadcrumbTrail.length > 1
                ? onBreadcrumbDragOver($event, breadcrumbTrail[breadcrumbTrail.length - 2].id)
                : null
            "
            (dragleave)="
              breadcrumbTrail.length > 1
                ? onBreadcrumbDragLeave($event, breadcrumbTrail[breadcrumbTrail.length - 2].id)
                : null
            "
            (drop)="
              breadcrumbTrail.length > 1
                ? onBreadcrumbDrop($event, breadcrumbTrail[breadcrumbTrail.length - 2].id)
                : null
            "
            [class.bg-blue-100]="
              breadcrumbTrail.length > 1 &&
              dragOverBreadcrumbId === breadcrumbTrail[breadcrumbTrail.length - 2].id
            "
          >
      ...
      <!-- Empty state -->
      <div
        *ngIf="!isLoading && folders.length === 0 && files.length === 0"
      ...
      >
        ...
        <p class="text-gray-400 text-xs">
          Glissez-déposez des fichiers ici ou créez un nouveau dossier
        </p>
      </div>
...
export class FileGridComponent {
  ...
  @Output() share = new EventEmitter<{ id: number; type: 'file' | 'folder'; name: string }>();
  ...
  onRenameFolder(folder: Folder) {
    this.showMenuFor = null;
    this.renameFolder.emit({ folder, newName: folder.name }); // Let parent handle the prompt/modal
  }
  ...
  onMoveFolder(folder: Folder) {
    this.showMenuFor = null;
    // Potentially emit a move request or show an internal picker
  }
